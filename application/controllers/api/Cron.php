...
public function seven_up()
{
        ...
        foreach ($room_data as $key => $room) {
            ...
            if ($game_data[0]->status == 0) {
                if ((strtotime($game_data[0]->added_date) + DRAGON_TIME_FOR_BET) <= time()) {
                    ...
                    // Randomize the outcome when both Up and Down bets are equal
                    if ($random == 1 || ($DownBetAmount == $UpBetAmount && $TieBetAmount == 0)) {
                        // Randomize between UP, DOWN, or TIE
                        $winning = rand(0, 2);  // 0 = UP, 1 = DOWN, 2 = TIE
                    } else {
                        // If Up and Down bets are not equal, then use your original logic
                        if ($DownBetAmount == 0 && $UpBetAmount == 0 && $TieBetAmount == 0) {
                            $winning = rand(0, 2);  // Randomize between UP, DOWN, or TIE
                        } elseif ($DownBetAmount > $TieBetAmount && $UpBetAmount > $TieBetAmount) {
                            $winning = TIE;
                        } else {
                            $winning = ($UpBetAmount > $DownBetAmount) ? DOWN : UP;
                        }
                    }

                    // Determine the number based on the winning bet
                    $winning_number = ($winning == DOWN) ? rand(2, 6) : (($winning == UP) ? rand(8, 12) : 7);
                    ...
                    // Update game data with the result
                    $update_data['status'] = 1;
                    $update_data['winning'] = $winning;
                    $update_data['total_amount'] = $TotalBetAmount;
                    $update_data['admin_profit'] = $TotalBetAmount - $TotalWinningAmount;
    ...
    public function roulette_winner_socket()
    {
        ...
        if ($room_data) {
            foreach ($room_data as $key => $room) {
                $game_data = $this->Roulette_model->getActiveGameOnTable($room->id);
	$incoming_data = "---- Incoming Game data ----\n";
	  $incoming_data .= "Game Data: " . print_r($game_data, true) . "\n";

     file_put_contents('/var/www/html/kuber7star.com/application/logs/debug_log.txt', $incoming_data, FILE_APPEND);
                ...
                if ($game_data[0]->status==0) {
                    // if ((strtotime($game_data[0]->added_date)+DRAGON_TIME_FOR_BET)<=time()) {
                        ...
                        // Give winning Amount to Number user
                        $bets = $this->Roulette_model->ViewBet("", $game_data[0]->id, $number);
 $incoming_data = "Bets Data: " . print_r($bets, true) . "\n";
file_put_contents('/var/www/html/kuber7star.com/application/logs/debug_log.txt',$incoming_data, FILE_APPEND);
...
$data = print_r("No winning bet Found", true);
file_put_contents('/var/www/html/kuber7star.com/application/logs/debug_log.txt',$data, FILE_APPEND);
...
if(!empty($update_data['admin_profit'])){
    ...
    public function check_payment_status()
    {
        ...
        foreach ($order_details as $key => $value) {
            ...
            if($response_arr['payment_status']=='finished'){
