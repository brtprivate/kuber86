...
class Plan extends REST_Controller
{
    ...
    public function Place_Order_NowPayment_post()
    {
       ...
       $user= $this->Users_model->UserProfile($user_id);
        ...
        $Order_ID = $this->Coin_plan_model->GetCoin($user_id, 0, $coin, $Amount);
    ...
    public function Place_Order_Upi_Gateway_post()
    {
      ...
      //   $Order_ID = $this->Users_model->GetCoin($user_id,0,0,$amount);
        ...
        curl_setopt_array($curl, array(
          ...
          "redirect_url": "https://games.androappstech.in/api/Callback",
    ...
    public function Place_Order_PgPay_post()
    {
        $user_id = $this->input->post('user_id');
        $amount = $this->input->post('amount');
        $plan_id = $this->input->post('plan_id');
      ...
      //   $Order_ID = $this->Users_model->GetCoin($user_id,0,0,$amount);
        ...
        {
          "partner_id":90,
          "apitxnid":1121'.$Order_ID.',
          "amount":'.$amount.',
          "callback": "https://nikatbygames.com/api/Callback"
        }
        ',
          CURLOPT_HTTPHEADER => array(
        ...
        // curl_close($curl);
        $response_arr = json_decode($response,true);
        if($response_arr['statuscode']!='TXN'){
            $data['message'] = $response_arr['message'];
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        ...
        $Update_Order_Master = $this->Coin_plan_model->UpdateOrder($user_id, $Order_ID, $response_arr['txnid']);
        if ($Update_Order_Master) {
            $data['order_id'] = $Order_ID;
            $data['Total_Amount'] = $amount;
            $data['txnId'] = $response_arr['txnid'];
            $data['clientRefId'] = '';
            $data['intentData'] = $response_arr['upi_string'];
            // $data['upi_id'] = $response_arr['data']['VPA'];
            $data['message'] = 'Success';
            $data['code'] = HTTP_OK;
            $this->response($data, 200);
