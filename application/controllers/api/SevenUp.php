...
class SevenUp extends REST_Controller
{
    ...
    public function get_active_game_socket_post()
    {
        ...
        if ($game_data) {
         ...
         //   $data['online'] = $this->SevenUp_model->getRoomOnline($this->data['room_id']);
            $data['online_users'] = $this->SevenUp_model->getRoomOnlineUser($this->data['room_id']);
            $data['online'] = min(rand(300, 1350), 1350);
            // $data['my_down_bet'] = $this->SevenUp_model->TotalBetAmount($game_data[0]->id, 0, $this->data['user_id']);
            // $data['my_up_bet'] = $this->SevenUp_model->TotalBetAmount($game_data[0]->id, 1, $this->data['user_id']);
            // $data['my_tie_bet'] = $this->SevenUp_model->TotalBetAmount($game_data[0]->id, 2, $this->data['user_id']);
            $data['last_bet'] = $this->SevenUp_model->ViewBet('', $game_data[0]->id, '', '', 1);
    ...
    public function place_bet_post()
    {
        ...
        if ($bet_id) {
            ...
            $my_down_bet=$this->SevenUp_model->TotalBetAmount( $this->data['game_id'], 0, $this->data['user_id']);
            $my_up_bet=$this->SevenUp_model->TotalBetAmount( $this->data['game_id'], 1, $this->data['user_id']);
            $my_tie_bet=$this->SevenUp_model->TotalBetAmount( $this->data['game_id'], 2, $this->data['user_id']);
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
            $user_wallet = $this->Users_model->UserProfile($this->data['user_id']);
            $data['wallet'] = $user_wallet[0]->wallet;
            $data['my_down_bet'] = $my_down_bet??0;
            $data['my_up_bet'] = $my_up_bet??0;
            $data['my_tie_bet'] = $my_tie_bet??0;
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
            exit();
        } else {
    ...
    public function get_total_bet_amount_post()
    {
        ...
        $my_up_bet = $this->SevenUp_model->get_dragon_total_bet($this->data['user_id'],$this->data['game_id']);
        $my_down_bet = $this->SevenUp_model->get_tiger_total_bet($this->data['user_id'],$this->data['game_id']);
        $my_tie_bet = $this->SevenUp_model->get_tie_total_bet($this->data['user_id'],$this->data['game_id']);

        if($my_up_bet || $my_down_bet || $my_tie_bet){
            $data['message'] = 'Success';
            $data['my_up_bet'] = $my_up_bet ?? '';
            $data['my_down_bet'] = $my_down_bet ?? '';
