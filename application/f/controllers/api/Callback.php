...
class Callback extends REST_Controller
{
    ...
    public function index_post()
    {
        ...
        $order_id=str_replace("1121","",$post->apitxnid);
        //param1 is mandatory
        if (empty($order_id)) {
        ...
        if ($post->statuscode=='TXN' && $order_details[0]->payment==0) {
            ...
            $this->Users_model->UpdateSpin($order_details[0]->user_id, ceil($post->amount/100));
    ...
    public function index_get()
    {
        // $post_data_expected = file_get_contents("php://input");
        // $post_data_expected = json_encode($_REQUEST);
        $post_data_expected='{"amount":"100","status":"success","statuscode":"TXN","txnid":"cust_Oa7l9hGRS99PA1","apitxnid":"112151","utr":"420065220360","payid":"389123"}';
        $data = [
            'response' => $post_data_expected
        ];
        $this->db->insert('response_log', $data);
    ...
    public function UpiGateway_post()
    {
        ...
        $post = json_decode($post_data_expected);
        $id=explode("=",explode("&",$post_data_expected)[0])[1];
        // $id = 80567325;
        //param1 is mandatory
        if (empty($id)) {
            $data['message'] = 'Invalid Order Id';
