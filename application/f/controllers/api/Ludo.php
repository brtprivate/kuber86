...
class Ludo extends REST_Controller
{
    ...
    public function get_table_master_post(){
        ...
        if(isset($this->data['no_of_players']) && !in_array($this->data['no_of_players'],array(2,3,4)))
        {
            $data['message'] = 'Invalid No. Of Players';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
		...
		$no_of_players = isset($this->data['no_of_players']) ? $this->data['no_of_players'] : 0;

        $table_data = $this->Ludo_model->getTableMaster('',$no_of_players);
