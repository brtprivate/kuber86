...
class Aviator_model extends MY_Model
{
    ...
    {
        $Query = $this->db->query('SELECT COUNT(`id`) as online FROM `tbl_aviator_bet` WHERE `dragon_tiger_id` = (SELECT `id` FROM `tbl_aviator` WHERE `room_id`='.$RoomId.' ORDER BY `id` DESC LIMIT 1)');
    ...
    {
        $ander_data = ['dragon_tiger_id' => $aviator_id, 'card' => $card, 'added_date' => date('Y-m-d H:i:s')];
    ...
    {
        return $this->db->where('dragon_tiger_id', $game_id)->where('user_id', $user_id)->delete('tbl_aviator_bet');
    }
    ...
    public function ViewBet($user_id='', $aviator_id='', $bet='', $bet_id='', $limit='')
    {
        ...
        if (!empty($aviator_id)) {
            $this->db->where('dragon_tiger_id', $aviator_id);
        }
    ...
    public function MakeWinner($user_id, $bet_id, $amount, $comission, $game_id)
    {
        ...
        log_statement ($user_id, AVIATOR, $user_winning_amt,
    ...
    {
        $this->db->select('tbl_aviator.*,(select count(id) from tbl_aviator_bet where tbl_aviator.id=tbl_aviator_bet.dragon_tiger_id) as total_users');
        $this->db->from('tbl_aviator');
        $this->db->order_by('id', 'DESC');
        $this->db->limit(10);
        $Query = $this->db->get();
