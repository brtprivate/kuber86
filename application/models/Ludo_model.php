...
class Ludo_model extends MY_Model
{
    ...
    public function getTableMaster($boot_value='',$no_of_players='')
    {
        ...
        $this->db->join('tbl_ludo_table', 'tbl_ludo_table_master.boot_value=tbl_ludo_table.boot_value AND tbl_ludo_table.isDeleted=0 AND `tbl_ludo_table`.`no_of_players` = '.$no_of_players, 'left');
    ...
    {
        $this->db->select('tbl_users.ludo_table_id,COUNT(tbl_ludo_table.id) AS members');
        $this->db->from('tbl_users');
        $this->db->join('tbl_ludo_table', 'tbl_users.ludo_table_id=tbl_ludo_table.id');
        $this->db->where('tbl_users.isDeleted', false);
    ...
    {
        $sql = "SELECT * FROM ( SELECT 1 AS mycolumn UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 ) a WHERE mycolumn NOT in ( SELECT seat_position FROM `tbl_ludo_table_user` WHERE ludo_table_id=" . $TableId . " AND isDeleted=0 ) LIMIT 1";
