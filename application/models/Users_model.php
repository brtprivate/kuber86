...
class Users_model extends MY_Model
{
    ...
    public function RegisterUser($MobileNo, $Name, $profile_pic, $gender = 'm', $token = '', $password = '', $bonus_amount = '', $app = 'self')
    {
        if (empty($profile_pic)) {
    ...
    public function Purchase_History()
    {
        $Query = $this->db->select('tbl_purchase.*,tbl_users.name')
            ...
            ->where('tbl_purchase.transaction_type', 0)
    ...
    public function ManualPurchase_History($status='')
    {
        $Query = $this->db->select('tbl_purchase.*,tbl_users.name')
            ...
            ->join('tbl_users', 'tbl_users.id=tbl_purchase.user_id')
            // ->where('tbl_purchase.payment', true)
            ->where('tbl_purchase.transaction_type', 1)
            ->where('tbl_purchase.status', $status)
            ->where('tbl_purchase.isDeleted', false)
            ->where('tbl_users.isDeleted', false)
            ->order_by('id','desc')
            ->get();
    ...
    {
        $Query = $this->db->query('
        SELECT tbl_game_log.`game_id`,
            SUM(tbl_game_log.`amount`) AS invest,
            IFNULL((SELECT user_winning_amt FROM `tbl_game` WHERE winner_id=' . $user_id . ' AND id=`game_id`), 0) AS winning_amount,
            tbl_game_log.added_date,
            tbl_table.private AS table_type
        FROM `tbl_game_log`
        JOIN tbl_game ON tbl_game_log.game_id=tbl_game.id
        JOIN tbl_table ON tbl_table.id=tbl_game.table_id
        WHERE tbl_game_log.`user_id`=' . $user_id . '
        GROUP BY tbl_game_log.`game_id`
        ORDER BY tbl_game_log.`game_id` DESC
    ');
    ...
    public function RummyLog($user_id)
    {
        ...
        UNION
        SELECT `game_id`,`user_id`,`action`,`amount`, 0 as user_winning_amt, 0 as admin_winning_amt,`added_date` FROM `tbl_rummy_log` WHERE `amount`!=0 AND `user_id`=' . $user_id . '
    ...
    {
        $Query = $this->db->query('SELECT * FROM
        (SELECT `id` as game_id,`winner_id` as user_id,"10" as action, amount , `user_winning_amt` as user_amount, `admin_winning_amt` as comission_amount,`added_date` FROM `tbl_poker` WHERE  `winner_id`=' . $user_id . ') poker
        UNION
    ...
    {
        $Query = $this->db->query('SELECT * FROM
        (SELECT `id` as game_id,`winner_id` as user_id,"10" as action, amount , `user_winning_amt` as user_amount, `admin_winning_amt` as comission_amount,`added_date` FROM `tbl_rummy_deal` WHERE  `winner_id`=' . $user_id . ') poker
        UNION
        SELECT `game_id`,`user_id`,`action`,`amount`, 0 as user_winning_amt, 0 as admin_winning_amt,`added_date` FROM `tbl_rummy_deal_log` WHERE `amount`!=0 AND `user_id`=' . $user_id . '
        ORDER BY added_date DESC');
        // $this->db->get();
        return $Query->result();
        // $Query = $this->db->query('SELECT * FROM
    ...
    {
        $Query = $this->db->query('SELECT * FROM
        (SELECT `id` as game_id,`winner_id` as user_id,"10" as action, amount , `user_winning_amt` as user_amount, `admin_winning_amt` as comission_amount,`added_date` FROM `tbl_rummy_pool` WHERE  `winner_id`=' . $user_id . ') poker
        UNION
        SELECT `game_id`,`user_id`,`action`,`amount`, 0 as user_winning_amt, 0 as admin_winning_amt,`added_date` FROM `tbl_rummy_pool_log` WHERE `amount`!=0 AND `user_id`=' . $user_id . '
        ORDER BY added_date DESC');
        // $this->db->get();
        return $Query->result();
        // $Query = $this->db->query('SELECT * FROM
...
//     }
    ...
    public function PurchaseChangeStatus($id, $status)
    {
        ...
        if ($status == 1) {
            ...
            $this->db->set('wallet', "wallet + $query->price", FALSE)
                ->set('unutilized_wallet', "unutilized_wallet + $query->price", FALSE)
                ->set('todays_recharge', "todays_recharge + $query->price", FALSE)
                ->set('updated_date', date('Y-m-d H:i:s'))
                ->where('id', $query->user_id)
                ->update('tbl_users');
        }
    ...
    public function getUserStatement($user_id)
    {
        $this->db->where('user_id', $user_id);  
        $this->db->order_by('id','desc');
